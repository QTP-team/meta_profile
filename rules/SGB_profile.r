...
profile <- function(uniq.filtered.dset, reads_cutoff = 0){
  ...
  SGB_r <- contig_r %>%
  left_join(select(genomeInfo, Genome_ID, Contig_ID), by = "Contig_ID") %>%
  group_by(Genome_ID) %>%
  ...
  left_join(SGB_size, by = "Genome_ID") %>%
...
write_tsv(uniq.filtered.dset, alignfile)
